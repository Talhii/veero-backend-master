 ...
 .toFile('../../../var/www/admin/images/'+image_name,(err)=>{
   if(!err)
   {
    ...
    .toFile('../../../var/www/trader/images/'+image_name,(err)=>
    {
      if(!err)
      {
        console.log('Image Saved to Trader Side');
      }
    })
   }
 });
...
router.get('/getimage/:page',ensureToken ,function(request, response) 
{
  ...
  pool.query(`SELECT COUNT(*) FROM banner_images;select * from banner_images ORDER BY Image_Id limit ${limit} OFFSET ${startNum} `, 
  function(err, result) 
  {
    if(err)
    {
